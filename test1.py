...
# Define BA_deps BEFORE it's used
...
async def main():
    ...
    4. You have to fetch the "projections" dict generated by BudgetProjectionTool and give it to RiskIdentificationTool as it is required for RiskIdentificationTool.
    5.Your output must be a JSON object with two keys: "projections" that holds the projected financial data and "risk_ranking" that holds the risk level (e.g., "low", "medium", "high").
    </agent_role>
    """
    ...
    deps = BA_deps(projections),
    
    # Fix the function signature to properly use context
    @BA_agent.tool
